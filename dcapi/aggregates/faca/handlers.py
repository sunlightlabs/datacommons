...
class FACAAgenciesHandler(EntityTopListHandler):
    ...
    stmt = """
        ...
        from agg_faca_records, (values (%s::uuid, %s::integer)) as params (entity_id, cycle)
        where
...
class FACACommitteeMembersHandler(TopListHandler):
    ...
    stmt = """
        ...
        from agg_faca_records, (values (%s::uuid, %s, %s::integer)) as params (entity_id, agency, cycle)
        where
            org_id = params.entity_id
