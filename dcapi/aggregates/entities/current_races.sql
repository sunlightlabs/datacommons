...
create table matchbox_currentrace as
    select 
        ...
        case
            when candidacy_status then 'G' 
            when not candidacy_status then 'P'
            else ''
    ...
    group by e.id, e.name, recipient_state, candidacy_status, district, seat, seat_status, seat_result;
