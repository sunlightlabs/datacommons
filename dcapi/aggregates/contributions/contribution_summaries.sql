...
create table summary_recipient_type_from_indiv as
      with individual_contributions_by_cycle as (
        select ca.entity_id as contributor_entity, me.name as contributor_name, recipient_type, cycle, count(*), sum(c.amount) as amount
    ...
    group by contributor_entity, contributor_name, recipient_type, cycle, count, amount

    )

     union all

     select contributor_entity, contributor_name, recipient_type, -1, count, amount, rank() over (partition by recipient_type order by sum(amount) desc) as rank
     from (
         select contributor_entity, contributor_name, recipient_type, sum(count) as count, sum(amount) as amount
         from individual_contributions_by_cycle
         group by contributor_entity, contributor_name, recipient_type
     ) x
    group by contributor_entity, contributor_name, recipient_type, count, amount;
